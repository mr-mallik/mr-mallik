    ...
    return(
        <div className="flex flex-col gap-10 mt-10 desktop:mt-20">
            <div className="flex flex-col gap-2">
                <h1 className="font-bold mb-6 uppercase tracking-wider underline underline-offset-8 color-tertiary laptop:text-2xl desktop:text-4xl">Honours & achievements</h1>
                    ...
                    {other.map((oth, idx) => {
                        return(
                            <div key={`hna-${idx}`} className="flex flex-nowrap justify-center content-center px-2 p-2 bg-gray-800 border rounded border-transparent hover:border-color-tertiary">
                                {/* <a href={(oth.link) ? oth.link : 'javascript:;'} target={(oth.link) ? '_blank' : ''} > */}
                                    <div className="rounded flex flex-col tablet:flex-row overflow-hidden shadow-md bg-gray-900 max-h-80">
                                        <div className="overflow-hidden bg-black  w-full tablet:w-2/5 flex justify-center items-center">
                                            <img alt="certificate" className="w-full desktop:h-32" src={(oth.logo) ? `/assets/other/` + oth.logo : `/assets/other/trophy.png`} />
                                        </div>
